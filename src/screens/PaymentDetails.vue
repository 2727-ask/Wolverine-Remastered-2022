<template>
  <div class="uk-container main-frame">
    ...
    <div class="uk-child-width-1-2@s uk-child-width-1-3@m uk-text-center" uk-grid>
      ...
      <div>
        <div class="uk-card uk-card-default uk-card-body">
          <input type="month" class="uk-input" v-model="dateyearstamp" @change="getDateYear" />
        </div>
      </div>
      <div>
        <div class="uk-card uk-card-default uk-card-body">
          <div uk-grid>
            ...
            <div style="margin-left:3px"><router-link
                :to="{ name: 'Print', query: { name:`${doctor_name}`,year:`${year}`,month:`${month}`}}">Print</router-link>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="uk-overflow-auto uk-height-large" style="margin-top: 20px">
      <table class="uk-table uk-table-striped uk-table uk-text-nowrap">
        ...
        <tbody>
          <tr v-for="payment in $store.state.newrecords.paymentRecord" :key="payment">
            <td>{{ makeDateReadable(payment.date) }}</td>
            <td>{{ payment.patient_name }}</td>
            <td>{{ payment.facility_name }}</td>
            <td>{{ payment.cut }}</td>
            <td>
              <a uk-icon="icon: more-vertical" :id="payment.id" @click="toggleModal(payment)"></a>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  <div id="modal-example" ref="modal_ref" uk-modal>
    <div class="uk-modal-dialog uk-modal-body">
      ...
      <p class="uk-text-right">
        ...
        <button class="uk-button uk-button-primary" @click="makeUpdatePaymentRequest"
          :disabled="$store.state.newrecords.isUpdating">
          <div v-if="$store.state.newrecords.isUpdating" uk-spinner="ratio :0.8"></div>
      ...
      <p>
        ...
        <slide-unlock style="margin-top:5vh" ref="vueslideunlock" :auto-width="true" :circle="true" :disabled="false"
          :noanimate="false" :width="600" :height="60" text="slide to delete" success-text="Successfully Deleted"
