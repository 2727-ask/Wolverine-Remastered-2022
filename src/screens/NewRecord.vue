<template>
  <div class="uk-container" style="margin-top: 5vh">
    <div class="uk-card uk-align-center uk-card-default uk-card-body uk-width-1-2@m">
      <div class="uk-card uk-align-center uk-card-primary uk-card-body" style="z-index: 0px">
        <h3>New Record</h3>
      </div>
      <form class="uk-grid-small" uk-grid @submit.prevent="submitForm">
        <div class="uk-width-1-1">
          <input class="uk-input" type="text" placeholder="Patient Name" v-model="patient_name" required />
        </div>
        <div class="uk-width-1-2@s">
          <input class="uk-input" type="number" placeholder="Age" v-model="age" required />
        </div>
        <div class="uk-width-1-2@s">
          <select class="uk-select" aria-placeholder="Gender" v-model="gender" required>
            <option>Male</option>
        ...
        <div class="uk-width-1-1@s">
          <input class="uk-input" type="date" placeholder="Date" v-model="date" required />
        </div>
        <div class="uk-width-1-2@s">
        ...
        <div class="uk-width-1-2@s">
          <label><input class="uk-checkbox" v-model="isCutNA" @change="activate_donot_pay_cut" type="checkbox" /><i
              style="margin-left: 30px">Do Not Pay Cut</i></label>
        </div>
        <div class="uk-width-1-1@s" style="width: 100%">
          <button class="uk-button uk-button-primary" style="width: 100%" type="text">
            Save
          </button>
        </div>
      </form>
  ...
  <div id="modal-overflow" ref="modal" uk-modal>
    <div class="uk-modal-dialog">
      ...
      <div class="uk-modal-footer uk-text-right">
        ...
        <button class="uk-button uk-button-primary" @click="makeSaveRequest"
          :disabled="$store.state.newrecords.isSaving">
          <div v-if="$store.state.newrecords.isSaving" uk-spinner="ratio :0.8"></div>
          <div v-else>Save</div>
        </button>
      </div>
    </div>
